...
PHP."\n", file_get_contents($expectedPath));
    ...
    public function test_class_names_that_collide_with_imports_are_rejected(string $name): void
    {
        $this->artisan('skir:make-request', [
            'method' => 'Admin.Users.GetUser',
            '--name' => $name,
        ])->expectsOutputToContain('Imported short name')->assertFailed();
    ...
    public function test_request_dto_collision_reports_the_actual_duplicate_short_name(): void
    {
        $definition = $this->definition(requestClass: 'App\\Skir\\ValidationRule');

        try {
            app(FormRequestScaffolder::class)->render($definition);
            self::fail('A request DTO import collision should fail.');
        } catch (SkirScaffoldingException $exception) {
            self::assertSame(
                'Imported short name [ValidationRule] is declared more than once in the generated form request.',
    ...
    {
        $filesystem = app(ScaffoldingFilesystem::class);
        ...
        $this->app->instance(AtomicFilePublisher::class, $publisher);
        $destinationPath = $this->temporaryDirectory.'/app/Http/Requests/Skir/Admin/Users/GetUserFormRequest.php';
        ...
        self::assertSame([], glob(dirname($destinationPath).'/.skir-*') ?: []);
    }
    ...
    public function test_publisher_leaves_temporary_file_cleanup_to_the_scaffolder(): void
    {
        $directory = $this->temporaryDirectory.'/app/Http/Requests';
        mkdir($directory, 0777, true);
        $temporaryPath = $directory.'/.skir-owner';
    ...
    public function test_cleanup_failure_after_publication_reports_created_destination_and_leftover_temp(): void
    {
        $filesystem = new ScaffoldingFilesystem(
            static function (string $operation, string $path): void {
                if ($operation === 'remove') {
        ...
        try {
            app(FormRequestScaffolder::class)->scaffold($this->definition());
            self::fail('A cleanup failure after publication should be reported.');
        } catch (SkirScaffoldingException $exception) {
            self::assertStringContainsString('was created, but temporary file cleanup failed', $exception->getMessage());
        }

        $requestDirectory = $this->temporaryDirectory.'/app/Http/Requests/Skir/Admin/Users';
        self::assertFileExists($requestDirectory.'/GetUserFormRequest.php');
        $temporaryFiles = glob($requestDirectory.'/.skir-*') ?: [];
        self::assertCount(1, $temporaryFiles);
        self::assertStringContainsString($temporaryFiles[0], $exception->getMessage());
    }

    public function test_directory_creation_race_succeeds_when_another_process_creates_the_directory(): void
    {
        $filesystem = new ScaffoldingFilesystem(
            static function (string $operation, string $path): void {
                if ($operation === 'makeDirectory') {
                    ...
                    throw new RuntimeException('Directory already created by another process.');
                }
            },
        );
        $path = $this->temporaryDirectory.'/race/directory';
    ...
    public function test_remove_race_is_idempotent_when_another_process_removes_the_file(): void
    {
        $path = $this->temporaryDirectory.'/race-remove';
        file_put_contents($path, 'temporary');
        $filesystem = new ScaffoldingFilesystem(
            static function (string $operation, string $operationPath): void {
                if ($operation === 'remove') {
                    unlink($operationPath);

                    throw new RuntimeException('File already removed by another process.');
                }
            },
        );
    ...
    public function test_builtin_filesystem_warning_is_wrapped_with_its_previous_error(): void
    {
        $path = $this->temporaryDirectory.'/missing.stub';

        try {
            (new ScaffoldingFilesystem)->read($path);
            self::fail('Reading a missing file should fail.');
        } catch (SkirScaffoldingException $exception) {
            self::assertStringContainsString('Filesystem operation [read] failed', $exception->getMessage());
            self::assertInstanceOf(ErrorException::class, $exception->getPrevious());
        }
    }

    private function definition(?string $requestClass = 'App\\Skir\\Admin\\GetUserRequestData'): SkirMethodDefinition
    {
